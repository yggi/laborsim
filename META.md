...
do not estimate it — and print it *before* the commit: a later round ran `wc` in
the same command that committed the trim and shipped `NOTES.md` at 104 claiming
it was done. A merge re-earned it at 105, both branches having landed on exactly
100. Spill on purpose rather than under that pressure.
