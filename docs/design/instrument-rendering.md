...
like industrial equipment from three different suppliers. `MEMORY.md` § 9 says
...
mechanic (`MEMORY.md` § 9) — gets worse before it gets better; and the draw-call
...
1. **Touch is the mechanic.** Direct manipulation, thumb reach, no hover, no
   pixel precision (`MEMORY.md` § 9). The DOM gives correct touch targets,
