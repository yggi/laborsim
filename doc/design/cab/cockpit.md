...
Spilled from `doc/MEMORY.md` § 6. Source: `HANDOVER.md` § 2.4, § 9.1–9.2, plus the
...
(`src/cockpit/cage.ts`, and the arms in `doc/design/cab/components.md`).
...
- **The flow is of *parts*, and it is bottom-aligned.** Every plate and every
  ...
  the masters are a group. See `doc/design/cab/components.md` for both mistakes and
...
enforced rather than written down (`doc/META.md`): `tests/cockpit.test.ts` fails if
