...
Companion to `doc/design/cab/components.md`, which says what the *parts* are. This
says how to *make* one. Rendering technology and why it is DOM at all:
`doc/design/cab/instrument-rendering.md`.
...
- **`mark`** — one SVG path on a 16×16 grid. One path, deliberately: a
  constraint that makes a fussy logo hard to write (`doc/META.md`, on naming rules
...
- **`voice`** — the maker's own strings: the warranty notice when its safety kit
  ...
  L.A.B.O.R.'s (`doc/design/rig/training-frame.md`).
...
This is not optional infrastructure. `doc/META.md` says it twice — *screenshots
...
1. **The maker's entry in `doc/LORE.md`** — its temperament, its doctrine, and
   above all *how its kit fails*, because that is what the player learns.
2. **`doc/design/cab/components.md`** — the invariants and the freedoms.
...
authors produce a coherent visual system from a written contract?** `doc/META.md`
