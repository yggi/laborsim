...
(L-050). The crystallized half is `doc/MEMORY.md` § 6.1; how to *make* a theme is
`doc/design/cab/theming.md`.
...
The three makers in `doc/LORE.md` are three different currencies, and this is the
...
live in the debrief (`doc/design/cab/cockpit.md`).
...
The table and the panel-language rules behind it: `doc/design/cab/theming.md`. The
...
- **a voice** — short house strings. Two consumers already exist: the warranty
  ...
  safety tips. L.A.B.O.R. keeps its own voice (`doc/design/rig/tone.md`); this is
...
`doc/design/cab/cockpit.md`, which owns that decision.
...
- This is **not** `CSS3DRenderer` and does not reopen
  `doc/design/cab/instrument-rendering.md`. It is a 2D translate driven by look
...
3. **Conformance is by accessible name, not by CSS review.** Every plate renders
   ...
   A rule enforced by a test, not by a document (`doc/META.md`).
...
6. **No new dependencies. No `:global`. No unprefixed custom properties.**
   ...
   SVG — in that order of preference (`doc/design/cab/instrument-rendering.md`).
...
- Whether a component may ship a pod its maker designed to be *dash-only* — a
  ...
  would never. Probably a component-curriculum hook (`doc/MEMORY.md` § 7) rather
