...
story rather than a feed. The target is set below `doc/MEMORY.md`'s on purpose:
...
What is built is in `README.md`; what it is, in `doc/MEMORY.md`. The one shape worth
...
and every attribution card waits on it. Recent detail is in `doc/LOG.md`.
